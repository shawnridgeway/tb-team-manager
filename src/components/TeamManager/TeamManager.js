...
class TeamManager extends React.Component {
  ...
  renderMembers = () => {
    return this.props.selectedMembers
        ...
        .map(memberId => {
          const member = this.props.allMembers
              .reduce((res, mem) => mem.id === memberId ? mem : res, null);
          if (!member) {
            return null;
          }
          const button = (
  ...
  render() {
    return (
        <Card>
          ...
          {(this.props.selectedMembers && this.props.selectedMembers.length) > 5 && (
              <div className="teamManager-showMore bg-light"
                   onClick={this.showMore}>
                {this.state.moreOpen
                    ? <React.Fragment>Show Less <i className="icon icon-chevron-up text-muted"/></React.Fragment>
                    : <React.Fragment>Show More <i className="icon icon-chevron-down text-muted"/></React.Fragment>
                }
              </div>
          )}
        </Card>
